...
# ═══════════════════════════════════════════════════════════════════════════════
# CSS FIXES (Ensuring Sidebar visibility and White Text for Options)
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown(f"""
    <style>
        ...
        if not data.empty:
            ...
            try:
                ...
                if model_mode == "Auto ARIMA":
                    model = pm.auto_arima(train_series, seasonal=False, error_action='ignore', suppress_warnings=True)
                    fc_vals = model.predict(n_periods=forecast_horizon)
                    order, aic = model.order, model.aic()
                else:
                    # Manual (fixed at 1,1,1 for testing, can expand to sliders)
                    fit = ARIMA(train_series, order=(1,1,1)).fit()
                    fc_vals = fit.forecast(steps=forecast_horizon)
                    order, aic = (1,1,1), fit.aic

                # 5. Inversion (The critical step to fix "None" values)
                last_p = raw_prices.iloc[-1]
                if transformation == "Log Returns":
                    forecast_prices = last_p * np.exp(np.cumsum(fc_vals))
                elif transformation == "Log Prices":
                    forecast_prices = np.exp(fc_vals)
                elif transformation == "Percentage Returns":
                    forecast_prices = last_p * (1 + np.cumsum(fc_vals))
                else:
                    forecast_prices = fc_vals
                
                # 6. Prepare Results
                f_dates = pd.date_range(raw_prices.index[-1], periods=forecast_horizon + 1, freq=resample_map[freq])[1:]
                # Ensure the values are passed as a clean list/array to the DataFrame
                fc_df = pd.DataFrame({"Forecasted Price": np.array(forecast_prices)}, index=f_dates)
...
if results:
    with tab1:
        ...
        fig.add_trace(go.Scatter(x=results["fc_df"].index, y=results["fc_df"]["Forecasted Price"], name="ARIMA Forecast", line=dict(color='orange')))
    ...
    st.info("Click 'FETCH DATA & RUN MODEL' to generate values.")
